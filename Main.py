...
ctypes.windll.kernel32.SetConsoleTitleW(
    f'[Alucard Selfbot v{SELFBOT.__version__}] | Loading...')
...
def startprint():
    ...
    print(f'''{Fore.RESET}
                      ...
                      ░  ░  ░    ░        ░   ▒   ░           ░   ▒ ▒ ░░     
                       ...
                       {Fore.CYAN}Stacey Selfbot {SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Alucard.user.name}#{Alucard.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Alucard.user.id}   
...
async def on_connect():
    ...
    if privnote_sniper == True:
        ...
        startprint()
        ctypes.windll.kernel32.SetConsoleTitleW(
            f'[Alucard Selfbot v{SELFBOT.__version__}] | Logged in as {Alucard.user.name}')
...
async def cuttly(ctx, *, link):  # b'\xfc'
    ...
    if cuttly_key == '':
        print(
            f"{Fore.RED}[ERROR]: {Fore.YELLOW}Cutt.ly API key has not been set in the config.json file"+Fore.RESET)
    else:
        try:
            req = requests.get(
                f'https://cutt.ly/api/api.php?key={cuttly_key}&short={link}')
            r = req.json()
            new = r['url']['shortLink']
            em = discord.Embed()
            em.add_field(name='Shortened link', value=new, inline=False)
...
async def whois(ctx, *, user: discord.Member = None):  # b'\xfc'
    ...
    if len(user.roles) > 1:
        ...
        em.add_field(name="Roles [{}]".format(
...
async def blank(ctx):  # b'\xfc'
    ...
    if config.get('password') == 'password-here':
        print(
            f"{Fore.RED}[ERROR] {Fore.YELLOW}You didnt put your password in the config.json file"+Fore.RESET)
    else:
        password = config.get('password')
...
async def tokenfuck(ctx, _token):  # b'\xfc'
    ...
    while True:
        ...
        while True:
            try:
                request.patch("https://canary.discordapp.com/api/v6/users/@me/settings",
                              headers=headers, json=setting, timeout=10)
            except Exception as e:
                print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)
            else:
                break
...
async def masscon(ctx, _type, amount: int, *, name=None):  # b'\xfc'
    ...
    for _i in range(amount):
        try:
            ...
            time.sleep(5)
            r = requests.put(
                f'https://canary.discordapp.com/api/v6/users/@me/connections/{_type}/{ID}', data=json.dumps(payload), headers=headers)
...
async def _auto_bump(ctx, channelid):  # b'\xfc'
    ...
    while True:
        try:
            ...
            print(
                f'{Fore.BLUE}[AUTO-BUMP] {Fore.GREEN}Bump number: {count} sent'+Fore.RESET)
...
async def mac(ctx, mac):  # b'\xfc'
    ...
    em = discord.Embed(title='MAC Lookup Result',
                       description=r.text, colour=0xDEADBF)
...
async def btc(ctx):  # b'\xfc'
    ...
    em.set_author(
        name='Bitcoin', icon_url='https://cdn.pixabay.com/photo/2013/12/08/12/12/bitcoin-225079_960_720.png')
    await ctx.send(embed=em)
...
async def eth(ctx):  # b'\xfc'
    ...
    em = discord.Embed(description=f'USD: `{str(usd)}$`\nEUR: `{str(eur)}€`')
    em.set_author(
        name='Ethereum', icon_url='https://cdn.discordapp.com/attachments/271256875205525504/374282740218200064/2000px-Ethereum_logo.png')
    await ctx.send(embed=em)
...
async def proxies(ctx):  # b'\xfc'
    ...
    res = requests.get(
        'https://api.proxyscrape.com/?request=displayproxies&proxytype=http&timeout=1500')
    proxies = []
